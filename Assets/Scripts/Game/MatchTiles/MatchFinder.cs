...
namespace Game.MatchTiles
{
    public class MatchFinder
    {
        ...
        public bool CheckBoardForMatches(IGrid grid)
        {
            ...
            for (int x = 0; x < grid.Width; x++)
            {
                for (int y = 0; y < grid.Height; y++)
                {
                    ...
                    MatchResult matchTiles = FindConectedTiles(tile, grid);
        ...
        private MatchResult FindConectedTiles(Tile tile, IGrid grid)
        {
        ...
        //удаление по Horizontal
            ...
            CheckDirection(tileGridPos, Vector2Int.left, grid,
                tile, connectedTiles);
            ...
            CheckDirection(tileGridPos, Vector2Int.up, grid,
                tile, connectedTiles);

            CheckDirection(tileGridPos, Vector2Int.down, grid,
                tile, connectedTiles);
        ...
        private MatchResult CheckForMultiResult(List<Tile> connectedTiles,
            IGrid grid, Vector2Int direction, MatchDirection matchDirection)
        {
            foreach (var tile in connectedTiles)
            {
                ...
                CheckDirection(grid.WorldToGrid(position), direction,
                    grid, tile, multiConnectedTiles);

                CheckDirection(grid.WorldToGrid(position), direction * -1,
                    grid, tile, multiConnectedTiles);
        ...
        private void CheckDirection(Vector2Int position, Vector2Int direction,
            IGrid grid, Tile tile, List<Tile> connectedTiles)
        {
