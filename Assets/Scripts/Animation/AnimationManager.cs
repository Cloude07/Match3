...
namespace Animation
{
    public class AnimationManager : IAnimation, IDisposable
    {
        ...
        public async UniTask Reveal(GameObject target, float delay)
        {
            ...
            await target.transform.DOScale(Vector3.one, delay)
        ...
        {
            _cts = new CancellationTokenSource();
            await target.transform.DOScale(Vector3.zero, 0.04f)
                   .SetEase(Ease.OutBounce);
        ...
        public void DoPunchAnimate(GameObject targer, Vector3 scale, float duration)
        {
            targer.transform.DOPunchScale(scale, duration, 1, 0.5f);
        }
